...
import { mapBond, type BondVM } from '../utils/adapter'
...
async function fetchBonds() {
  ...
  try {
    ...
    const data = res.data
    const items = Array.isArray(data) ? data as unknown as Bond[] : (data.items ?? [])
    bonds.value = items.map((b) => mapBond(b as unknown as Record<string, unknown>))
